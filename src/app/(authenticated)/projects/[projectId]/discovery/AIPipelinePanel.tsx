...
import { runAutoRecon } from "@/lib/actions/recon";
...
export function AIPipelinePanel({ projectId, evidenceStats, hasArtifact, latestVersion }: AIPipelinePanelProps) {
  ...
  const handleRecon = () => {
    ...
    startTransition(async () => {
      try {
        const result = await runAutoRecon(projectId);
        if (result.error) {
          toast.error("Recon failed", result.error);
        } else if (result.skipped) {
          toast.warning("Duplicate", "Recon data already ingested for this domain.");
        } else {
          toast.success("Recon complete", `${result.findings} signals found, ${result.chunkCount} chunks created.`);
          router.refresh();
        }
      } catch (err) {
        toast.error("Recon failed", err instanceof Error ? err.message : String(err));
      } finally {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: "var(--foreground-dim)" }}>
              Auto-scan DNS, HTTP headers, cloud topology, and auth patterns — ingested as evidence.
            </p>
          </div>
          <button
            onClick={handleRecon}
            disabled={isPending || reconRunning}
            className="btn-secondary text-sm py-2 px-4 disabled:opacity-50 inline-flex items-center gap-2"
          >
            {reconRunning ? (
              <>
